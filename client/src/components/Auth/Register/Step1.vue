  ...
  <ValidationObserver ref="observer" v-slot="{ invalid, validate }">
    <form @submit.prevent="next({ firstName, lastName, country })">
      <b-field class="mb-5" label="First Name">
        <BInputWithValidation v-model="firstName" rules="required"></BInputWithValidation>
      </b-field>
      <b-field class="mb-5" label="Last Name">
        <BInputWithValidation v-model="lastName" rules="required"></BInputWithValidation>
      </b-field>
      <b-field class="mb-5" label="Country">
        <b-select class="dropdown-box"
            v-model="country"
            icon="globe-americas"
            icon-pack="fas"
            placeholder="Country"
